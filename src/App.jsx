...
function App() {
  ...
  return (
    <div className="h-full flex flex-col text-left min-w-[1280px] w-[1480px] m-auto ">
      
      <div className='py-4 px-4 gap-4 border-b border-gray-200'>
...
function Table ({rows, schema, pageSize=10, onRowClicked}) {
  ...
  function downloadCSV () {
    ...
    rows.map(row => {
      csvContent.push(schema.map(s => row[s.colname]).join(','));
    })
  ...
  rows.map((row,i) => row.i =i);
  const maxPage = Math.ceil(rows.length / pageSize);
  const pages = [];
  while (pages.length < maxPage) pages.push(pages.length + 1);
  ...
  return <div className="border border-gray-200 rounded-md text-sm">
      ...
      <tbody>
        {pageData.map(row => {
          return <tr className="cursor-pointer hover:bg-gray-100/50 text-xs" onClick={() => onRowClicked && onRowClicked(row)}>
            {schema.map(col => {
              if (row[col.colname]===null)  return <td className={col.align +" text-gray-400 italic"}> null</td>
              if (row[col.colname]===undefined)  return <td className={col.align +" text-gray-400 italic"}> undefined</td>
              if (row[col.colname]==="")  return <td className={col.align +" text-gray-400 italic"}> empty string</td>
              if (row[col.colname]==="Multiple Values")  return <td className={col.align +" text-gray-400 italic"}> Multiple Values</td>
              if (row[col.colname]===false)  return <td className={col.align +" text-gray-400 italic"}> false</td>
              if (row[col.colname]===true)  return <td className={col.align +" text-gray-400 italic"}> true</td>
              return <td className={col.align}> {row[col.colname]}</td>
            })}
          </tr>
        })}
      </tbody>
